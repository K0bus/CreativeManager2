...
public class ClaimProtection extends Protection {
    ...
    private boolean notMember(Player player, Location location) {
        ...
        if (getPlugin().getServer().getPluginManager().isPluginEnabled("GriefPrevention")
                ...
                && isMemberGP(player, location)) return false;
        return true;
    }
    ...
    private boolean isMemberWG(Player player, Location location) {
        ...
        Set<ProtectedRegion> regions =
                query.getApplicableRegions(BukkitAdapter.adapt(location)).getRegions();
        for (ProtectedRegion region : regions) {
            if (region.isMember(localPlayer)
                    && query.testState(BukkitAdapter.adapt(location), localPlayer, Flags.BUILD)) return true;
        }
        return false;
    }
    ...
    private boolean isMemberLands(Player player, Location location) {
        ...
        if (area != null) return area.isTrusted(player.getUniqueId());
        return false;
    }

    private boolean isMemberGP(Player player, Location location) {
        Claim claim = GriefPrevention.instance.dataStore.getClaimAt(location, false, null);
        if (claim != null) return claim.getOwnerID().equals(player.getUniqueId());
        return false;
    }
}
